...
export default function Home() {
  ...
  const getAllTodos = async () => {
    try {
      ...
      if (Array.isArray(response.data.todoItem)) {  
  ...
  const insertTodo = async (todoDescription: string) => {
    ...
    try {
      const response = await axios.post(API_URL, { todoDescription }, {
        headers: { 'Content-Type': 'application/json' }
      });
  
      console.log("Inserted Todo Response:", response.data); 
  
      if (response.data?.todoItem) { 
        const newTodo = { 
          _id: response.data.todoItem.id, 
          todoDescription: response.data.todoItem.todoDescription 
        };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(`${API_URL}/${editId}`, { todoDescription: editText });
      ...
      if (response.data.todoItem) {
        setTodos((prevTodos) =>
          prevTodos.map((todo) =>
            todo._id === editId ? { ...todo, todoDescription: response.data.todoItem.todoDescription } : todo
          )
        );
      }
    ...
    <div className="w-full h-screen flex justify-center">
      <div className="bg-blue-300 max-h-full p-20 w-[600px]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {inputerror &&
            <p id="inputerror" className="text-[12px] text-red-600 pl-14 font-bold">
